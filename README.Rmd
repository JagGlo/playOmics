...
BRCA data has been downloaded from LinkedOmics portal and are freely available for the research purpose: https://www.linkedomics.org/data_download/TCGA-BRCA/.
...
For the purpose of logging, it's profitable to define working directory, either by setwd() function or by the usage of here package.
...
-   filter missing data - similar to above, but the variables are removed based upon % of missing values (e.g. variables with more than 50% of missing values will be removed):  
...
Initially we had 504 columns. After filtering for non-missing values in at least of 50% columns we end up with 451 columns.
...
!!!! install.packages("mlr3filters", version='0.6.0') -? due to missing values handling
...
W opisie wspomnieć że przez to że datasety się różnie łączą to nie zawsze mamy idealną sytuację i komplet badań. czasem mamy tylko np proteomikę dla 100 pacjentów z 1000 i wtedy nie musimy tych danych odrzucać!!!


Nieoptymalne zapisywanie danych - BRCA_data 550 MB, BRCA_data_splitted - 450 MB, data prepared - 360 MB, modelling_set - 450 MB, splitted_data - 500 MB -? razem prawie 3 GB!!!
```{r}
test_data_prepared <-
