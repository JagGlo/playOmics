...
Let's split the data randomly with proportion 90%/10%. We will leave the 10% test data as a simulation for new experiment after the modelling process is done.
...
We will treat the `splitted_data$train_data` as a background for next experiments.
...
``` r
data_prepared <-
    prepare_data_for_modelling(data = splitted_data$train_data, target = target)
```


``` r
BRCA_data_splitted <- 
  split_data_into_train_test(data_prepared, prop = 8/10, target = target)
```
...
``` r
model_data(data_filtered, test_data, experiment_name, 3)
